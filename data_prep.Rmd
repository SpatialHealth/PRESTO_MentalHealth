...
data = data_full[c("StudyID", "Country", "state",  "ageatqstn", "b_race_ethnic_census", "b_educ", "b_income", "ct_HHinc", "ct_lths", "ct_urban", "pss_score", "b_mdi_score",  "b_finisheddate", 'b_walkwork', 'b_walkexer', 'b_walktrans', 'b_bike', 'b_freeweights', 'b_jogswimraq', 'b_aerobics', 'b_garden', 'b_livebirths', 'b_everpregnant', 'ttp_entry', 'geoflag', 'geoflag_b', #'b_smoke', 'b_beerfreq', 'b_redwinefreq', 'b_whitewinefreq', 'b_dessertwinefreq','b_liqourfreq',
'b_unemp', 
...
# impute covariates 
imp_vars <- c("b_income", "b_totalmet", "ct_HHinc", "ct_lths", "ct_urban_cat", "b_livebirths", "ttp_entry", "b_unemp")
imputed <- hotdeck(data,
                   variable = imp_vars,
...
# specify columns
confounding <- c("ageatqstn", "b_race_ethnic_census", "b_educ", "b_income", "b_totalmet", "ct_HHinc", "ct_lths", "ct_urban_cat")
...
ct <- exercise %>% dplyr::filter(complete.cases(select(., all_of(c("ct_urban_cat", "ct_lths", "ct_HHinc")))))
...
```{r, Drop Participants with missing data}
confounding <- c("ageatqstn", "b_race_ethnic_census", "b_educ", "b_income", "b_totalmet", "ct_HHinc", "ct_lths", "ct_urban_cat", "season")
outcomes <- c("pss_score", "b_mdi_score")
exposures <- names(data)[grep("ndvi", names(data))]

imputed_final <- imputed |> tidyr::drop_na(all_of(c(confounding, outcomes, exposures))) |> dplyr::filter(!(geoflag_b %in% c(4,5,6,7,9)))
imputed_dropped <- subset(imputed, !(imputed$StudyID %in% imputed_final$StudyID))

complete_final <- data |> tidyr::drop_na(all_of(c(confounding, outcomes, exposures))) |> dplyr::filter(!(geoflag_b %in% c(4,5,6,7,9)))
...
factor_covs <- c("b_race_ethnic_census", "b_educ", "b_income", "ct_urban_cat", "b_unemp", "b_everpregnant", "season")
